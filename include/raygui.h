...
bool GuiImageButtonEx(Rectangle bounds, const char *text, Texture2D texture, Rectangle texSource)
{
    ...
    if (text != NULL)
        GuiDrawText(text, GetTextBounds(BUTTON, bounds), GuiGetStyle(BUTTON, TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(BUTTON, TEXT + (state * 3))), guiAlpha));
    if (texture.id > 0)
        DrawTextureRec(texture, texSource, RAYGUI_CLITERAL(Vector2){bounds.x + bounds.width / 2 - texSource.width / 2, bounds.y + bounds.height / 2 - texSource.height / 2}, Fade(GetColor(GuiGetStyle(BUTTON, TEXT + (state * 3))), guiAlpha));
    //------------------------------------------------------------------
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize, bool editMode)
{
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked)
    {
        ...
        if (pressed)
            framesCounter = 0; // Reset blinking cursor
    }
    //--------------------------------------------------------------------
...
int GuiListView(Rectangle bounds, const char *text, int *scrollIndex, int active)
{
    ...
    if (text != NULL)
        items = GuiTextSplit(text, &itemsCount, NULL);
    ...
    if (rgiFile != NULL)
    {
            ...
            (signature[3] == ' '))
        {
            if (loadIconsName)
            {
                ...
                fseek(rgiFile, iconsCount * RICON_MAX_NAME_LENGTH, SEEK_CUR);
